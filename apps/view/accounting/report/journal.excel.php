...
while ($data = $report->FetchAssoc()) {
    ...
    $sheet->setCellValue("F$row", $showNo ? $data["acdebet_no"] : $data["acdebet_no"].' - '.$data["acc_debit"]);
    $sheet->setCellValue("G$row", $data["jumlah"]);
    $sheet->setCellValue("H$row", $showNo ? $data["ackredit_no"] : $data["ackredit_no"].' - '.$data["acc_credit"]);
