...
$sheet->setCellValue("D6", "Cabang");
$sheet->setCellValue("E6", "Debet");
$sheet->setCellValue("F6", "Kredit");
$sheet->getStyle("A6:F6")->applyFromArray(array(
...
$sheet->setCellValue("E7", ($haveData && $openingBalance->GetCoa()->PosisiSaldo == "DK") ? $transaction["saldo"] : 0);
$sheet->setCellValue("F7", ($haveData && $openingBalance->GetCoa()->PosisiSaldo == "KD") ? $transaction["saldo"] : 0);
...
while ($data = $report->FetchAssoc()) {
	...
	$debit = $data["acdebet_no"] == $accountNo ? $data["jumlah"] : 0;
	$credit = $data["ackredit_no"] == $accountNo ? $data["jumlah"] : 0;
	...
	$sheet->setCellValue("D$row", $data["kd_cabang"]);
...
$sheet->mergeCells("A$row:D$row");
$sheet->setCellValue("E$row", $selectedAccount->PosisiSaldo == "DK" ? "=E" . ($row - 1) . "-F" . ($row - 1) : "");
$sheet->setCellValue("F$row", $selectedAccount->PosisiSaldo == "KD" ? "=F" . ($row - 1) . "-E" . ($row - 1) : "");
$sheet->getStyle("A$row:F$row")->applyFromArray(array(
	"font" => array("bold" => true),
	"borders" => array(
		"bottom" => array("style" => PHPExcel_Style_Border::BORDER_THIN)
...
$sheet->getStyle("E6:E$row")->getNumberFormat()->setFormatCode('#,##0.00');
